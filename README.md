# 🏠 DormFix — ระบบแจ้งซ่อมหอพัก

ระบบจัดการแจ้งซ่อมสำหรับหอพัก พัฒนาด้วย React + Node.js + MySQL  
รองรับ 3 บทบาท: **ผู้พักอาศัย**, **ช่างซ่อม**, และ **ผู้ดูแลระบบ (Admin)**

🌐 **Live Demo**: [https://project-dormfix.up.railway.app](https://project-dormfix.up.railway.app)

---

## ✨ ฟีเจอร์หลัก

- 🔐 ระบบ Login / Register 3 บทบาท (ผู้พักอาศัย / ช่างซ่อม / แอดมิน)
- 📝 แจ้งซ่อมพร้อมแนบรูปภาพ
- 🔧 ช่างรับงาน / อัปโหลดรูปหลังซ่อม / ยกเลิกงาน (พร้อมเหตุผล)
- 💬 ระบบแชทภายในแต่ละคำร้อง
- ⭐ ผู้พักอาศัยให้คะแนนและรีวิวช่าง
- 📊 Dashboard แยกตามสถานะ (Pending / In-Progress / Resolved)
- 🏆 ดูรีวิวรายช่างพร้อมคะแนนเฉลี่ย
- 👤 แก้ไขโปรไฟล์ (ชื่อ, รูปภาพ, รหัสผ่าน, เบอร์โทร)
- 🛡️ แอดมินจัดการผู้ใช้ / สร้างบัญชี / เปลี่ยนบทบาท
- 🌗 สลับธีมมืด/สว่าง (Dark/Light Mode)

---

## 📦 Tech Stack

| ส่วน | เทคโนโลยี |
|------|-----------|
| **Frontend** | React 19, Vite 7, Tailwind CSS v4, Axios, Lucide React, React Router v7 |
| **Backend** | Node.js, Express.js, Multer (อัปโหลดรูป), mysql2/promise |
| **Database** | MySQL (utf8mb4) |
| **Deployment** | Railway (Node.js + MySQL) |

---

## 🚀 การติดตั้ง (Local Development)

### ขั้นตอนที่ 1: Clone โปรเจกต์

```bash
git clone https://github.com/Beyondeva/Project-webtechnolohy.git
cd Project-webtechnolohy
```

### ขั้นตอนที่ 2: Import ฐานข้อมูล

- เปิด **phpMyAdmin**: [http://localhost/phpMyAdmin](http://localhost/phpMyAdmin)
- คลิก **Import** → เลือกไฟล์ `database.sql` → กด **Import**
- ระบบจะสร้าง database `dorm_maintenance` พร้อมตาราง + demo users

### ขั้นตอนที่ 3: ตั้งค่า Database

แก้ไขไฟล์ `server.js` ให้ตรงกับ MySQL ของคุณ:

```js
const pool = mysql.createPool({
  host: process.env.MYSQLHOST || 'localhost',
  port: process.env.MYSQLPORT || 3306,
  user: process.env.MYSQLUSER || 'root',
  password: process.env.MYSQLPASSWORD || 'YOUR_PASSWORD',  // ← แก้ตรงนี้
  database: process.env.MYSQLDATABASE || 'dorm_maintenance',
});
```

### ขั้นตอนที่ 4: ติดตั้ง Dependencies

```bash
# Backend
npm install

# Frontend
cd client
npm install
```

### ขั้นตอนที่ 5: รันระบบ

เปิด **2 Terminal** พร้อมกัน:

```bash
# Terminal 1 — Backend (Port 5000)
node server.js

# Terminal 2 — Frontend (Port 5173)
cd client
npm run dev
```

เปิดเบราว์เซอร์ที่ **[http://localhost:5173](http://localhost:5173)**

---

## 👤 บัญชีทดลองใช้งาน (Demo Users)

| Username | Password | บทบาท |
|----------|----------|-------|
| `user1` | `pass1` | ผู้พักอาศัย |
| `user2` | `pass2` | ผู้พักอาศัย |
| `user3` | `pass3` | ผู้พักอาศัย |
| `tech1` | `pass1` | ช่างซ่อม |
| `tech2` | `pass2` | ช่างซ่อม |
| `admin` | `admin123` | ผู้ดูแลระบบ |

---

## 📂 โครงสร้างโปรเจกต์

```
PJWEB/
├── server.js            ← Backend API (Express.js)
├── database.sql         ← SQL สร้างฐานข้อมูล
├── package.json         ← Backend dependencies + build script
├── .gitignore
├── uploads/             ← รูปภาพที่อัปโหลด (สร้างอัตโนมัติ)
├── logo/                ← โลโก้โปรเจกต์
└── client/              ← Frontend (React + Vite)
    ├── public/
    │   └── logo.png         ← Favicon & Logo
    ├── src/
    │   ├── api.js             ← Axios config (ใช้ env var)
    │   ├── App.jsx            ← Routes
    │   ├── index.css          ← Tailwind styles
    │   ├── context/
    │   │   ├── AuthContext.jsx ← จัดการ Login state
    │   │   └── ThemeContext.jsx← จัดการธีม (มืด/สว่าง)
    │   ├── hooks/
    │   │   └── useThemeClasses.js ← Hook สำหรับ class ธีม
    │   ├── components/
    │   │   ├── Navbar.jsx     ← แถบนำทาง + โลโก้ + ปุ่มเปลี่ยนธีม
    │   │   ├── TicketChat.jsx ← ระบบแชทใน Ticket
    │   │   ├── StarRating.jsx ← ดาวให้คะแนน
    │   │   └── ProtectedRoute.jsx
    │   └── pages/
    │       ├── Login.jsx       ← หน้าเข้าสู่ระบบ
    │       ├── Register.jsx    ← หน้าสมัครสมาชิก
    │       ├── Dashboard.jsx   ← หน้าแดชบอร์ด (แยกตาม status)
    │       ├── CreateTicket.jsx← หน้าแจ้งซ่อม
    │       ├── TicketDetail.jsx← หน้ารายละเอียดคำร้อง + แชท
    │       ├── Users.jsx       ← หน้าจัดการผู้ใช้ (Admin)
    │       └── Profile.jsx     ← หน้าโปรไฟล์
    └── package.json    ← Frontend dependencies
```

---

## 🌐 Deployment (Railway)

โปรเจกต์นี้ deploy บน [Railway](https://railway.app) ซึ่งรวม Node.js + MySQL ในที่เดียว

### Environment Variables ที่ใช้

| Variable | ใช้โดย | คำอธิบาย |
|----------|--------|---------|
| `MYSQL_URL` | Backend | Connection string สำหรับ MySQL |
| `PORT` | Backend | Port ที่ Railway กำหนด (auto) |
| `VITE_API_URL` | Frontend | URL ของ API (ว่าง = same origin) |

### วิธี Deploy

1. สร้างโปรเจกต์บน Railway → เชื่อม GitHub repo
2. เพิ่ม MySQL service → Reference `MYSQL_URL` variable
3. รัน `database.sql` เพื่อสร้างตาราง
4. กด **Generate Domain** ใน Settings → Networking

---

## 📜 API Endpoints

| Method | Endpoint | คำอธิบาย |
|--------|----------|---------|
| POST | `/api/login` | เข้าสู่ระบบ |
| POST | `/api/register` | สมัครสมาชิก |
| GET | `/api/tickets` | ดู Tickets ทั้งหมด |
| POST | `/api/tickets` | สร้าง Ticket ใหม่ |
| GET | `/api/tickets/:id` | ดูรายละเอียด Ticket |
| PUT | `/api/tickets/:id/assign` | มอบหมายช่าง |
| PUT | `/api/tickets/:id/status` | เปลี่ยนสถานะ |
| PUT | `/api/tickets/:id/rate` | ให้คะแนน + รีวิว |
| PUT | `/api/tickets/:id/cancel` | ยกเลิก Ticket |
| GET | `/api/tickets/:id/messages` | ดูข้อความแชท |
| POST | `/api/tickets/:id/messages` | ส่งข้อความแชท |
| GET | `/api/technician-ratings` | คะแนนช่างทั้งหมด |
| GET | `/api/technician-reviews/:id` | รีวิวรายช่าง |
| GET | `/api/users` | ดูผู้ใช้ทั้งหมด (Admin) |
| POST | `/api/users` | สร้างผู้ใช้ (Admin) |
| PUT | `/api/users/:id` | แก้ไขผู้ใช้ |
| PUT | `/api/users/:id/role` | เปลี่ยนบทบาท (Admin) |
| PUT | `/api/profile/:id` | แก้ไขโปรไฟล์ |
